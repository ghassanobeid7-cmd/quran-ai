...
function getSectionText(entry) {
  ...
  if (entry && Array.isArray(entry.tafsir_sections)) {
    return normalizeText(entry.tafsir_sections[0]?.tafsir || "");
  }
  return "";
}
...
function splitTransitionText(text) {
  ...
  const transitionRegex = /(?:\sثم\s+|\s)(?:تفتتح|تبدأ|◆\s*بداية)\s+سورة\s+/u;
...
async function main() {
  ...
  for (let page = 151; page <= 200; page += 1) {
  ...
  console.log("تم إصلاح الصفحات 151-200 بصيغة tafsir_sections مع نص سطر واحد.");
}
