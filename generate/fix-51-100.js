...
function getSectionText(entry) {
  ...
  if (entry && Array.isArray(entry.tafsir_sections)) {
    return entry.tafsir_sections.map((s) => String(s.tafsir || "").trim()).filter(Boolean).join(" ");
  }
  return "";
}
...
async function main() {
  ...
  for (let page = 51; page <= 100; page += 1) {
  ...
  console.log("تم إصلاح الصفحات 51-100 بصيغة tafsir_sections مع زيادة تنسيق الأسطر.");
}
